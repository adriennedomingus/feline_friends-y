...
RSpec.describe Cart, type: :model do
  ...
  context "tells if the cart contains cats" do
    it "returns true if the cart has cats" do
      cat = create_cats[:cat1]
      cart = Cart.new({})
      cart.add_cat(cat.id)
      expect(cart.contains_cats?).to be_truthy
    end
    it "returns false if the cart is empty" do
      cart = Cart.new({})
